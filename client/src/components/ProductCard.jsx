...
const ProductCard = ({ product }) => {
  ...
  const createdAtTime = product.createdAt ? new Date(product.createdAt).getTime() : 0;
  const isNewProduct = product.isNewArrival || (createdAtTime && Date.now() - createdAtTime < 1000 * 60 * 60 * 24 * 14);
  ...
  const productForCheckout = {
    ...product,
    price: displayPrice,
    selectedSize: sizePrice?.size || product.size?.[0] || product.sizes?.[0] || 'Free Size',
  };
    ...
    >
      <Link to={`/product/${product._id}`} className="block">
        <div className="relative aspect-[3/4] overflow-hidden">
          ...
          >
            <Heart size={18} className={isWishlisted ? 'fill-accent text-accent' : 'text-white'} />
          </button>
