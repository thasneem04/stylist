...
const Cart = () => {
  ...
  const subtotal = cart.reduce((acc, item) => acc + (item.price * (1 - (item.discount || 0)/100)) * item.quantity, 0);
  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-20 flex flex-col items-center justify-center text-center">
        ...
        <h2 className="text-3xl font-black uppercase tracking-tight mb-4">Your cart is empty</h2>
        <p className="text-gray-400 mb-8 max-w-md">Looks like you haven't added any items to your cart yet. Discover your next favorite outfit.</p>
        <Link to="/products" className="bg-gradient-btn px-8 py-4 rounded-xl font-bold uppercase tracking-wider flex items-center justify-center gap-2 text-sm">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 sm:py-12">
      <h1 className="mb-6 text-2xl font-black uppercase tracking-tight sm:mb-8 sm:text-3xl">Shopping <span className="text-gradient">Cart</span></h1>

      <div className="flex flex-col gap-8 lg:flex-row lg:gap-12">
        {/* Cart Items */}
        <div className="w-full lg:w-2/3 space-y-6">
          {cart.map((item) => (
            <div key={item._id} className="glass-card relative flex flex-col items-center gap-4 p-4 sm:flex-row sm:gap-6 sm:p-6">
              <img src={getImageUrl(item.image)} alt={item.name} className="h-36 w-28 rounded-lg object-cover sm:h-32 sm:w-24" />
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-sm text-gray-400 mb-2">Size: {item.selectedSize} | Color: <span className="capitalize">{item.color}</span></p>
                <div className="flex flex-wrap items-center justify-center gap-3 sm:justify-start sm:gap-4">
                  ...
                  <span className="text-xs text-gray-500 bg-white/5 px-2 py-1 rounded">Qty: {item.quantity}</span>
                </div>
              </div>
              <button 
        ...
        <div className="w-full lg:w-1/3">
          <div className="glass-card sticky top-32 p-5 sm:p-6 lg:top-24">
            ...
            <div className="flex justify-between items-center mb-8 pt-4 border-t border-white/10">
              ...
              <span className="text-2xl font-black text-gradient">₹{total.toFixed(2)}</span>
            </div>
