...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      try {
        ...
        if (data.image?.includes('unsplash')) throw new Error('Product removed');
        setProduct({ ...data, id: data._id });
        setSelectedSize(data.sizePrices?.[0]?.size || data.sizes?.[0] || 'Free Size');
        ...
        if (Array.isArray(allProducts)) {
          const matchingProducts = allProducts.filter(item => (
            item._id !== data._id &&
            (item.category === data.category || item.occasion === data.occasion)
          ));
          const fallbackProducts = allProducts.filter(item => item._id !== data._id);
          setRelatedProducts((matchingProducts.length ? matchingProducts : fallbackProducts).slice(0, 8));
        }
      } catch {
        setProduct(false);
        setRelatedProducts([]);
  ...
  if (product === null) return <div className="p-20 text-center">Loading...</div>;
  if (product === false) return <div className="p-20 text-center">Product not found. Explore the collection for more styles.</div>;

  const isWishlisted = wishlist.some(item => item.id === product.id);
  const productSizes = product.sizePrices?.length ? product.sizePrices.map(item => item.size) : (product.size || product.sizes || []);
  const selectedSizePrice = product.sizePrices?.find(item => item.size === selectedSize);
  const currentPrice = selectedSizePrice?.price ?? product.price;
  const currentOriginalPrice = selectedSizePrice?.originalPrice ?? product.price;
  ...
  const handleWishlist = () => {
    ...
    toast(isWishlisted ? 'Removed from Wishlist' : 'Added to Wishlist', { icon: '❤️' });
  };

  return (
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 sm:py-12">
      <Link to="/products" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-white mb-8 transition-colors">
      ...
      <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-[minmax(280px,420px)_1fr] lg:gap-14">
        ...
        <div className="w-full flex flex-col pt-4">
          ...
          <h1 className="mb-4 text-3xl font-black tracking-tight md:text-5xl">{product.name}</h1>
          ...
          <p className="text-gray-300 leading-relaxed mb-10 max-w-lg">
            Experience premium quality with this stunning {product.name.toLowerCase()}. Designed specifically for {product.occasion} occasions, featuring advanced fabric technology.
          </p>

          {/* Color Selection */}
          <div className="mb-8">
            <h3 className="text-sm font-bold uppercase tracking-wider text-gray-400 mb-3">Color</h3>
            <div className="flex items-center gap-3">
              <div 
                className="w-10 h-10 rounded-full border-2 border-accent p-1"
              >
                <div className="w-full h-full rounded-full" style={{ backgroundColor: productColor }} />
              </div>
              <span className="text-sm capitalize font-medium">{productColor}</span>
            </div>
          </div>
          ...
          <div className="mb-10">
            <div className="flex justify-between items-center mb-3 max-w-md">
              <h3 className="text-sm font-bold uppercase tracking-wider text-gray-400">Size</h3>
          ...
          <div className="flex max-w-xl flex-col gap-3 sm:flex-row sm:gap-4">
            ...
            >
              <Heart size={24} className={isWishlisted ? 'fill-accent text-accent' : 'text-white'} />
            </button>
          </div>
        ...
        <section className="mt-20">
          <div className="flex items-end justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-gray-400 mt-2">More styles from the same collection and occasion.</p>
            </div>
            <Link to="/products" className="hidden sm:inline-flex text-sm font-bold uppercase tracking-wider text-accent hover:text-white">
              View All
            </Link>
          </div>
