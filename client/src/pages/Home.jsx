...
const fallbackOccasionImages = {
  ...
  office: 'https://source.unsplash.com/900x900/?modest,office,professional,outfit',
...
const Home = () => {
  ...
  useEffect(() => {
    const loadHeroProduct = async () => {
      try {
        ...
        setHeroProduct(Array.isArray(newProducts) && newProducts.length > 0 ? newProducts[0] : null);
      } catch {
        setHeroProduct(null);
      }
    };
  ...
  const heroImage = heroProduct?.image ? getImageUrl(heroProduct.image) : heroFallback;
  const heroProductPath = heroProduct?._id ? `/product/${heroProduct._id}` : '/products';
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col items-center gap-10 px-4 text-center sm:px-6 lg:flex-row lg:gap-12 lg:text-left">
          ...
          >
            ...
            <p className="mx-auto mb-8 max-w-xl text-base text-gray-300 sm:text-lg lg:mx-0 lg:mb-10">
              Personalized fashion recommendations curated by advanced AI. Experience fashion that truly understands your unique style.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
              <Link to="/products" className="w-full sm:w-auto bg-gradient-btn px-8 py-4 rounded-xl font-bold uppercase tracking-wider flex items-center justify-center gap-2 text-sm">
                Shop Collection <ArrowRight size={18} />
              </Link>
              <button onClick={() => document.getElementById('chatbot-toggle')?.click()} className="w-full sm:w-auto glass px-8 py-4 rounded-xl font-bold uppercase tracking-wider hover:bg-white/10 transition-colors text-sm">
                Chat with Aria
              </button>
            </div>
          </motion.div>
          ...
          >
            <Link to={heroProductPath} className="block w-full max-w-[420px] aspect-[4/5] rounded-[1.5rem] border border-white/10 shadow-[0_0_40px_rgba(138,43,226,0.28)] overflow-hidden relative bg-black/30 group">
              <img src={heroImage} alt={heroProduct?.name || 'Featured fashion style'} className="w-full h-full object-contain bg-black/20" />
              ...
              <div className="absolute left-6 right-6 bottom-6">
                ...
                <p className="text-sm text-gray-300 mt-3 line-clamp-2">
                  {heroProduct?.description || 'Discover polished styles for everyday confidence.'}
                </p>
        ...
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center divide-y md:divide-y-0 md:divide-x divide-white/10">
            <div className="flex flex-col items-center justify-center p-4">
              ...
              <p className="text-gray-400 uppercase tracking-widest text-xs font-bold">Premium Products</p>
            </div>
            <div className="flex flex-col items-center justify-center p-4">
              <Smile size={32} className="text-accent-purple mb-4" />
              <h3 className="text-4xl font-black text-white mb-2">98%</h3>
              <p className="text-gray-400 uppercase tracking-widest text-xs font-bold">Happy Customers</p>
            </div>
            <div className="flex flex-col items-center justify-center p-4">
              <Clock size={32} className="text-accent mb-4" />
              <h3 className="text-4xl font-black text-white mb-2">24/7</h3>
              <p className="text-gray-400 uppercase tracking-widest text-xs font-bold">AI Support</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative z-10 py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tight mb-4">Shop by <span className="text-gradient">Occasion</span></h2>
            <p className="text-gray-400">Find the perfect outfit for your next event</p>
          </div>
            ...
            {occasions.map((occ, idx) => (
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                  <h3 className="text-xl md:text-2xl font-black uppercase tracking-wider text-white group-hover:text-accent transition-colors">{occ.name}</h3>
                  <span className="mt-3 text-[10px] font-bold uppercase tracking-[0.25em] text-gray-300">Explore</span>
                </div>
              </motion.button>
            ))}
          </div>
